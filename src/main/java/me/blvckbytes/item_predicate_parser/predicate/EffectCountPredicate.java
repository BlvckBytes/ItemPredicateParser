...
import me.blvckbytes.item_predicate_parser.translation.keyed.EffectCountKey;
import org.jetbrains.annotations.Nullable;

public record EffectCountPredicate(
  Token token,
  TranslatedLangKeyed<EffectCountKey> translatedLangKeyed,
  IntegerToken amountArgument
) implements ItemPredicate {

  @Override
  public @Nullable ItemPredicate testForFailure(PredicateState state) {
    if (amountArgument.matches(state.getEffects().size()))
      return null;
  ...
  public boolean equals(Object other) {
    if (!(other instanceof EffectCountPredicate otherPredicate))
      return false;
